                ...
                <tr class="border-b border-slate-200 dark:border-slate-700">
                    ...
                    @else
                        @foreach($headers as $header)
                            <th class="px-6 py-4 text-xs font-semibold text-slate-500 dark:text-slate-400 tracking-wider uppercase {{ $loop->last ? 'text-right' : '' }}">
                                {{ $header }}
                            </th>
                        @endforeach
                    @endif
    ...
    @if($paginator && $paginator->hasPages())
        <div class="p-4 border-t border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-4 bg-slate-50 dark:bg-[#0f172a]">
            ...
            <div class="flex items-center text-sm text-slate-600 dark:text-slate-400">
                <select 
                    class="bg-white dark:bg-[#0f172a] border border-slate-300 dark:border-slate-700 text-slate-700 dark:text-slate-300 text-sm rounded focus:ring-indigo-500 focus:border-indigo-500 py-1 pl-2 pr-6"
