...
<aside x-data="{ 
    ...
    <div class="flex items-center justify-between h-16 px-4 border-b border-slate-200 dark:border-slate-800">
        ...
        <!-- Toggle colapso (solo escritorio) -->
        <button @click="collapsed = !collapsed" 
                class="hidden lg:flex p-2 rounded-md text-slate-400 hover:text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800 focus:outline-none focus:ring-2 focus:ring-indigo-500"
    ...
    <!-- Navegación -->
    <nav id="sidebar-nav" class="flex-1 overflow-y-auto py-4" aria-label="Navegación principal">
        <div class="px-3 space-y-1">
            <!-- Dashboard -->
            <a href="{{ route('dashboard') }}" 
               class="flex items-center px-3 py-2.5 rounded-lg text-sm font-medium transition-colors group
                      {{ request()->routeIs('dashboard') 
                          ? 'bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 border-l-4 border-indigo-600' 
                          : 'text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800' }}"
               :class="collapsed ? 'justify-center' : ''"
               aria-current="{{ request()->routeIs('dashboard') ? 'page' : 'false' }}">
                <svg class="h-5 w-5 shrink-0" :class="collapsed ? '' : 'mr-3'" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" />
                </svg>
                <span class="truncate" :class="collapsed ? 'lg:hidden' : ''">{{ __('Dashboard') }}</span>
            </a>
                ...
                <a href="{{ route('categorias.index') }}" 
                   class="flex items-center px-3 py-2.5 rounded-lg text-sm font-medium transition-colors group
                          {{ request()->routeIs('categorias.*') 
                              ...
                              : 'text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800' }}"
                   :class="collapsed ? 'justify-center' : ''"
                   aria-current="{{ request()->routeIs('categorias.*') ? 'page' : 'false' }}">
                    <svg class="h-5 w-5 shrink-0" :class="collapsed ? '' : 'mr-3'" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z" />
                    </svg>
                    <span class="truncate" :class="collapsed ? 'lg:hidden' : ''">{{ __('Categorías') }}</span>
                </a>
            @endcan
                ...
                <a href="{{ route('bitacora.index') }}" 
                   class="flex items-center px-3 py-2.5 rounded-lg text-sm font-medium transition-colors group
                          {{ request()->routeIs('bitacora.*') 
                              ...
                              : 'text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800' }}"
                   :class="collapsed ? 'justify-center' : ''"
                   aria-current="{{ request()->routeIs('bitacora.*') ? 'page' : 'false' }}">
                    <svg class="h-5 w-5 shrink-0" :class="collapsed ? '' : 'mr-3'" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01" />
                    </svg>
                    <span class="truncate" :class="collapsed ? 'lg:hidden' : ''">{{ __('Bitácora') }}</span>
                </a>
            @endcan
            ...
            @can('viewAny', App\Models\Rol::class)
                <a href="{{ route('roles.index') }}" 
                   ...
                   aria-current="{{ request()->routeIs('roles.*') ? 'page' : 'false' }}">
                    ...
                    <span class="truncate" :class="collapsed ? 'lg:hidden' : ''">{{ __('Roles') }}</span>
                </a>
            @endcan
                ...
                <a href="{{ route('permisos.index') }}" 
                   class="flex items-center px-3 py-2.5 rounded-lg text-sm font-medium transition-colors group
                          {{ request()->routeIs('permisos.*') 
                              ...
                              : 'text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800' }}"
                   :class="collapsed ? 'justify-center' : ''"
                   aria-current="{{ request()->routeIs('permisos.*') ? 'page' : 'false' }}">
                    <svg class="h-5 w-5 shrink-0" :class="collapsed ? '' : 'mr-3'" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v2H7v2H4a1 1 0 01-1-1v-2.586a1 1 0 01.293-.707l5.964-5.964A6 6 0 1121 9z" />
                    </svg>
                    <span class="truncate" :class="collapsed ? 'lg:hidden' : ''">{{ __('Permisos') }}</span>
                </a>
            @endcan
            ...
            @can('viewAny', App\Models\User::class)
                <a href="{{ route('usuarios.index') }}" 
                   ...
                   aria-current="{{ request()->routeIs('usuarios.*') ? 'page' : 'false' }}">
                    ...
                    <span class="truncate" :class="collapsed ? 'lg:hidden' : ''">{{ __('Usuarios') }}</span>
                </a>
            @endcan
        </div>
    </nav>
        ...
        <x-dropdown align="left" width="48">
            <x-slot name="trigger">
                <button class="flex items-center w-full px-3 py-2 rounded-lg text-sm font-medium text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors focus:outline-none focus:ring-2 focus:ring-indigo-500"
                    ...
                    <span class="flex-1 text-left truncate ml-3" :class="collapsed ? 'lg:hidden' : ''">{{ Auth::user()->name }}</span>
                    <svg class="h-4 w-4 ml-2 shrink-0" :class="collapsed ? 'lg:hidden' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24">
