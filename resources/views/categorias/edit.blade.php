<x-app-layout>
    ...
    <div class="py-12" x-data="categoriaForm({{ $categoria->id_categoria }})">
            ...
            <x-card>
                <div class="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-4 mb-6">
                    <div>
                        ...
                        <p class="text-sm text-slate-500 dark:text-slate-400">Modifique los datos de la categoría seleccionada.</p>
                    </div>
                ...
                <div class="w-full">
                    <form method="POST" action="{{ route('categorias.update', $categoria) }}" novalidate @submit.prevent="submitForm($event)">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                            ...
                            <div>
                                ...
                                >{{ old('descripcion_categoria', $categoria->descripcion_categoria) }}</x-textarea>
                        ...
                        <div class="mt-6 flex flex-col-reverse sm:flex-row justify-end gap-3">
                            <a href="{{ route('categorias.index') }}" class="inline-flex items-center justify-center gap-1.5 px-4 py-2 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-500 rounded-md font-semibold text-xs text-gray-700 dark:text-gray-300 uppercase tracking-widest shadow-sm hover:bg-gray-50 dark:hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800 disabled:opacity-25 transition ease-in-out duration-150 w-full sm:w-auto">
    ...
    <script>
        function categoriaForm(categoriaId = null) {
            return {
                form: {
                    nombre_categoria: @js(old('nombre_categoria', $categoria->nombre_categoria)),
                    id_categoria_padre_categoria: @js(old('id_categoria_padre_categoria', $categoria->id_categoria_padre_categoria ?? '')),
                    descripcion_categoria: @js(old('descripcion_categoria', $categoria->descripcion_categoria ?? ''))
                },
                errors: {},
                dirtyFields: new Set(),
                isValidating: false,
                ...
                async validateForm() {
                    ...
                    try {
                        const response = await fetch("{{ route('api.system.validate-partial', ['formRequest' => 'Categoria/CategoriaFormRequest']) }}", {
                            method: "POST",
                            headers: {
                ...
                async submitForm(e) {
                    // Forzar validación final de todos los campos
