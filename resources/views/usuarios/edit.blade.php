            ...
            <x-card>
                <div class="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-4 mb-6">
                    <div>
                        ...
                        <p class="text-sm text-slate-500 dark:text-slate-400">Modifique los datos del usuario seleccionado.</p>
                    </div>
                ...
                <div class="w-full">
                    <form method="POST" action="{{ route('usuarios.update', $usuario) }}" novalidate @submit.prevent="submitForm($event)">
                        @csrf
                        @method('PUT')
        ...
        function userForm(userId = null) {
            return {
                ...
                async validateForm() {
                    ...
                    try {
                        const response = await fetch("{{ route('api.system.validate-partial', ['formRequest' => 'User/UserFormRequest']) }}", {
                            method: "POST",
                            headers: {
