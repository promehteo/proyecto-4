        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <div class="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
                    <form method="GET" action="{{ route('usuarios.index') }}" class="flex flex-wrap items-center gap-3 w-full md:w-auto">
                        <input type="text" name="search" value="{{ $search }}" placeholder="Buscar por nombre o correo..." class="rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm">
                    ...
                    @can('create', App\Models\User::class)
                        <a href="{{ route('usuarios.create') }}" class="inline-flex items-center px-4 py-2 bg-indigo-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-indigo-500">
                            + Nuevo Usuario
                        </a>
                    @endcan
                </div>
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                                ...
                                <tr>
                                    ...
                                    <td class="px-6 py-4 text-sm text-gray-500">
                                        @forelse($u->roles as $r)
                                            <span class="inline-block px-2 py-0.5 bg-indigo-100 text-indigo-800 text-xs font-semibold rounded mr-1 mb-1">
                                                {{ $r->nombre_rol }}
                                            </span>
                                        @empty
                                            <span class="text-xs text-gray-400">Sin roles activos</span>
                                        @endforelse
                                    </td>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <div class="flex justify-end gap-3">
                                            @can('assignRoles', $u)
                                                <a href="{{ route('usuarios.roles.edit', $u) }}" class="text-blue-600 hover:text-blue-900">Roles</a>
                                            @endcan

                                            @can('update', $u)
                                                <a href="{{ route('usuarios.edit', $u) }}" class="text-indigo-600 hover:text-indigo-900">Editar</a>
                                            @endcan

                                            @if($u->status === 1)
                                                @can('deactivate', $u)
                                                    <button type="button" @click="abrirConfirmacion('{{ route('usuarios.inactivar', $u) }}', '{{ $u->name }}')" class="text-red-600 hover:text-red-900">
                                                        Inactivar
                                                    </button>
                                                @endcan
                                            @else
                            ...
                            @empty
                                <tr>
                                    <td colspan="6" class="px-6 py-4 text-center text-sm text-gray-500">No se encontraron usuarios.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
        ...
        <div x-show="confirmModal" class="fixed inset-0 z-50 overflow-y-auto" style="display: none;">
            <div class="flex items-center justify-center min-h-screen px-4">
                ...
                <div class="bg-white rounded-lg overflow-hidden shadow-xl transform transition-all sm:max-w-lg sm:w-full p-6 z-10">
                    <h3 class="text-lg font-medium text-gray-900 mb-4">Confirmar Inactivación de Usuario</h3>
                    <p class="text-sm text-gray-500 mb-6">
                        ¿Está seguro que desea inactivar al usuario <strong x-text="nombreUsuario"></strong>? El usuario no se eliminará físicamente.
                    </p>
                    <div class="flex justify-end gap-3">
