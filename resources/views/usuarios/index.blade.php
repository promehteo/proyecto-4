    ...
    <div class="pt-6 pb-12" x-data="{
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <x-card>
                ...
                <x-data-table :paginator="$usuarios" :headers="['ID', 'NOMBRE Y APELLIDO', 'CÉDULA', 'CORREO ELECTRÓNICO', 'ROLES ASIGNADOS', 'ESTADO', 'ACCIONES']">
                    @forelse($usuarios as $u)
                        <tr>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-slate-900 dark:text-slate-100">#{{ $u->id_user }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-semibold text-slate-900 dark:text-slate-100">{{ $u->nombre }} {{ $u->apellido }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-900 dark:text-slate-100">{{ $u->cedula ?? 'N/A' }}</td>
                                ...
                                <div class="flex justify-end gap-2">
                                    <a href="{{ route('usuarios.show', $u) }}" class="px-3 py-1 inline-flex items-center text-xs font-bold rounded-md bg-blue-600 text-white hover:bg-blue-700 transition-colors duration-150 gap-1">
                                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                            ...
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-9.542 0-9.542-7-9.542-7z"></path>
                                        </svg>
                                        Ver
                                    </a>
                                    ...
                                    @if($u->status === 1)
                                        @can('deactivate', $u)
                                            <button type="button" @click="abrirConfirmacion('{{ route('usuarios.inactivar', $u) }}', '{{ $u->nombre }} {{ $u->apellido }}')" class="px-3 py-1 inline-flex items-center text-xs font-bold rounded-md bg-red-600 text-white hover:bg-red-500 transition-colors duration-150 gap-1">
                                                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-6 py-4 text-center text-sm text-slate-500">No se encontraron usuarios.</td>
                        </tr>
                    @endforelse
                </x-data-table>
            </x-card>
