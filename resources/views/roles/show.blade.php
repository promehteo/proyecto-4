<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Detalles del Rol: ') . $rol->nombre_rol }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            <x-card>
                ...
                <div class="border-b border-slate-200 dark:border-slate-800 pb-4 mb-6">
                    ...
                    <p class="text-sm text-slate-500 dark:text-slate-400">Detalles de registro del rol seleccionado.</p>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <!-- Nombre -->
                    <div>
                        <x-input-label :value="__('Nombre del Rol')" />
                    ...
                    <div class="md:col-span-3 mt-4">
                        ...
                        <div class="mt-2 flex flex-wrap gap-2">
                            @forelse($rol->permisos as $permiso)
                                <span class="px-2.5 py-1 rounded-md text-xs font-medium bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700">
                                    {{ $permiso->nombre_permiso }}
                                </span>
                            @empty
                                <p class="text-sm text-slate-500 italic">No tiene permisos asignados.</p>
                            @endforelse
                        </div>
                    </div>
                </div>
